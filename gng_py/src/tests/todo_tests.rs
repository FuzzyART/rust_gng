...
use crate::gas::core::sub_calc_neuron_err_ranking;
...
use crate::gas::core::init_model_ecs;

use crate::handlers::neuron_handler::NeuronHandler;
use crate::handlers::edge_handler::EdgeHandler;
use crate::handlers::config_handler::ConfigHandler;
//--------------------------------------------------------------------------------------------------
#[test]
pub fn remove_unconnected_neurons_t1(){
    let filename = "test_data/growing_neural_gas/remove_unconnected_neurons_t1.json";
...
//    // input
    ...
    gng_params.model_params.edge_start = json_reader::read_array_usize(&reader,"input","edge_start");
    gng_params.model_params.edge_end = json_reader::read_array_usize(&reader,"input","edge_end");
    gng_params.model_params.w =          json_reader::read_array_f64(&reader,  "input","W");

    gng_params.model_params.input_width = json_reader::read_val_usize(&reader, "input","input_width");
    gng_params.model_params.num_neurons = json_reader::read_val_usize(&reader, "input","num_neurons");
//    // input
//    //----------------------------------------
//    // output

    let target_w = json_reader::read_array_f64(&reader,          "output","W");
//    let target_num_neurons = json_reader::read_val_usize(&reader,"output","num_neurons");
...
//    // function call
//
                remove_unconnected_neurons(&mut gng_params);
//
//    // function call
//    //----------------------------------------
//    // asserts
...
//use crate::gas::core::sub_calc_best_neighbor_neuron;
...
//
//
//use crate::data_structures::params;
//use crate::gas::math;
...
//use crate::handlers::input_set_handler::Samples;
//use crate::handlers::config_handler::Config;
//use crate::handlers::input_set_handler::InputSetHandler;
...
//    //--------------------------------------------------------------------------------------------------
    ...
    fn calc_winner_edges_test() {
        ...
        gng_params.model_params.edge_start = json_reader::read_array_usize(&res_read, "input", "edge_start");
        gng_params.model_params.edge_end = json_reader::read_array_usize(&res_read, "input", "edge_end");
        gng_params.model_params.distance_ranking = json_reader::read_array_usize(&res_read, "output", "distance_ranking");
        gng_params.model_params.input_width = json_reader::read_val_usize(&res_read, "variables", "input_width");
        gng_params.model_params.num_neurons = json_reader::read_val_usize(&res_read, "variables", "num_neurons");
       ...
       // output
        ...
        for a in 0..winner_edges_target.len() {
            assert_eq!(gng_params.model_params.winner_edges[a], winner_edges_target[a]);
        }
    }
    //--------------------------------------------------------------------------------------------------
    #[test]
    pub fn calc_neuron_dependencies_t1() {
        ...
        //------------------------------------------------------------------------------
        // input
        gng_params.model_params.input_width = json_reader::read_val_usize(&reader, "variables", "input_width");
        gng_params.model_params.num_neurons = json_reader::read_val_usize(&reader, "variables", "num_neurons");

        gng_params.model_params.edge_start = json_reader::read_array_usize(&reader, "input", "edge_start");
        gng_params.model_params.edge_end = json_reader::read_array_usize(&reader, "input", "edge_end");
        gng_params.model_params.distance_ranking = json_reader::read_array_usize(&reader, "output", "distance_ranking");
        gng_params.model_params.w = json_reader::read_array_f64(&reader, "input", "w");
        gng_params.model_params.winner_edges = json_reader::read_array_usize(&reader, "output", "winner_edges");
        // input
        //------------------------------------------------------------------------------
        // output
    ...
    fn calc_distance_order_test_t1() {
        ...
        for a in 0..target_distance_order.len() {
            assert_eq!(gng_params.model_params.distance_order[a], target_distance_order[a] as usize);
        }
    }
 ...
 //       let mut algo_state: AlgorithmState = AlgorithmState::init();
    ...
    fn test_decrease_error_global_t1() {
        ...
        //------------------------------------------------------------------------------
        // input
        gng_params.model_params.neuron_err = json_reader::read_array_f64(&res_read, "input", "neuronErrC_In");
    ...
    fn increase_edge_age_t1() {
        ...
        gng_params.model_params.input_width = json_reader::read_val_usize(&res_read, "variables", "inputWidth");
        gng_params.model_params.num_neurons = json_reader::read_val_usize(&res_read, "variables", "numNeurons");
        gng_params.model_params.edge_start = json_reader::read_array_usize(&res_read, "input", "edges_start_rust");
        gng_params.model_params.edge_end = json_reader::read_array_usize(&res_read, "input", "edges_end_rust");
        gng_params.model_params.edge_age = json_reader::read_array_usize(&res_read, "input", "edges_age_rust");
        gng_params.model_params.distance_ranking = json_reader::read_array_usize(&res_read, "input", "distance_ranking");
        ...
        let target_edge_age   = json_reader::read_array_usize(&res_read, "output", "edges_age_target_rust");
        // output
        //----------------------------------------
    ...
    fn add_error_to_winner_neuron_t1() {
        ...
        gng_params.model_params.input_width = json_reader::read_val_usize(&res_read, "variables", "inputWidth");
        gng_params.model_params.w = json_reader::read_array_f64(&res_read, "input", "W_in");
        gng_params.model_params.neuron_err = json_reader::read_array_f64(&res_read, "input", "neuronErr_In");
        gng_params.model_params.distance = json_reader::read_array_f64(&res_read, "input", "neuronDist");
        gng_params.model_params.distance_order = json_reader::read_array_usize(&res_read, "input", "orderPos_In");

        gng_params.algo_state.sample_id_position = json_reader::read_val_usize(&res_read, "variables", "smpPos");

        gng_params.input_set_params.sample = json_reader::read_array_f64(&res_read, "input", "input_In");

        // input
        //----------------------------------------
        // output

        let neuron_err_target = json_reader::read_array_f64(&res_read, "output", "neuronErr_Target");
...
fn sub_calc_neuron_err_ranking_t1() {
    ...
    let target_neuron_err =
        json_reader::read_array_f64(&reader, "output", "neuron_err");
    ...
    sub_calc_neuron_err_ranking(&mut gng_params);
    ...
    for a in 0..target_neuron_err.len() {
        assert_eq!(
            target_neuron_err[a],
            gng_params.model_params.neuron_err[a]
        );
    }
}
#[test]
pub fn remove_old_edges_t1(){
    ...
    gng_params.model_params.edge_start = json_reader::read_array_usize(&reader,    "input","edge_start");
    gng_params.model_params.edge_end = json_reader::read_array_usize(&reader,      "input","edge_end");
    gng_params.model_params.edge_age = json_reader::read_array_usize(&reader,      "input","edge_age");
    gng_params.train_params.edge_removal_age = json_reader::read_val_usize(&reader,"input","edge_removal_age");
    // input
    //----------------------------------------
    // output
    let target_edge_start = json_reader::read_array_usize(&reader,"output","edge_start");
...
//    // asserts
    ...
    pub fn calc_distance_ranking_t1() {
        ...
        gng_params.model_params.distance_ranking = json_reader::read_array_usize(&reader, "input", "distance_ranking");
        gng_params.model_params.distance_order = json_reader::read_array_usize(&reader, "input", "distance_order");

        // input
        //------------------------------------------------------------------------------
        // output

        let target_distance_ranking = json_reader::read_array_usize(&reader, "output", "distance_ranking");

        // output
        //------------------------------------------------------------------------------
        // function call
        
        calc_distance_ranking(
            &mut gng_params,
        );
        ...
        for a in 0..target_distance_ranking.len() {
            assert_eq!(target_distance_ranking[a], gng_params.model_params.distance_ranking[a]);
        }
    }
    //--------------------------------------------------------------------------------------------------
    #[test]
    ...
    pub fn update_weights_t1() {
        ...
        // input
        gng_params.model_params.input_width = json_reader::read_val_usize(&reader, "input", "input_width");
        gng_params.train_params.epsilon_w = json_reader::read_val_f64(&reader,     "input", "epsilon_w");
        gng_params.train_params.epsilon_n = json_reader::read_val_f64(&reader,     "input", "epsilon_n");
        gng_params.model_params.w = json_reader::read_array_f64(&reader, "input", "w");
        gng_params.model_params.neuron_dependencies = json_reader::read_array_usize(&reader, "input", "neuron_dependencies");
        gng_params.model_params.distance_ranking = json_reader::read_array_usize(&reader, "input", "distance_ranking");
        gng_params.algo_state.curr_sample_pos = json_reader::read_val_usize(&reader, "input", "smp_pos");
        gng_params.input_set_params.sample = json_reader::read_array_f64(&reader, "input", "input_set");
        // input
        //------------------------------------------------------------------------------
        // output
